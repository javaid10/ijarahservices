...
public class ScoringEngine implements JavaService2 {
    ...
    public Object invoke(String s, Object[] objects, DataControllerRequest dataControllerRequest, DataControllerResponse dataControllerResponse) throws Exception {
        ...
        try {
            if (preProcess(inputParams)) {
                ...
                Result getSalaryCertificate = getSIMAHSalaryCertificate(inputParams, dataControllerRequest);
                if (getSalaryCertificate.getParamValueByName("bankCode").isEmpty() || IjarahHelperMethods.isBlank(getSalaryCertificate.getParamValueByName("bankCode"))) {
                    EMPLOYER_TYPE_ID = "3";
                    this.inputParams.put("employerTypeId", EMPLOYER_TYPE_ID);
                    getSalaryCertificate = getSIMAHSalaryCertificate(inputParams, dataControllerRequest);
                }
                ...
                Result getConsumerEnquiry = getSIMAHConsumerEnquiry(createRequestForConsumerEnquiryService(inputParams, getCustomerData, getSalaryCertificate, getNationalAddress), dataControllerRequest);
    ...
    private Map<String, String> createRequestForCreateCustomerAddressService(Result getNationalAddress) {
        ...
        try {
            ...
            LOG.error("ERROR createRequestForCreateCustomerAddressService :: " + ex);
        }
        return inputParams;
    }

    private Map<String, String> createRequestForConsumerEnquiryService(Map<String, String> globalInputParams, Result getCustomerData, Result getSalaryCertificate, Result getNationalAddress) {
        Map<String, String> inputParams = new HashMap<>();
        try {

            String expiryDate = String.valueOf(LocalDate.parse(HelperMethods.getFieldValue(getCustomerData, "IDExpiryDate"), DateTimeFormatter.ofPattern("dd/MM/yyyy")));
            DOB = String.valueOf(HelperMethods.getFieldValue(getCustomerData, "DateOfBirth"));
            String dateOfBirth = String.valueOf(LocalDate.parse(HelperMethods.getFieldValue(getCustomerData, "DateOfBirth"), DateTimeFormatter.ofPattern("dd/MM/yyyy")));
            ...
            inputParams.put("CID3", expiryDate);
            inputParams.put("CID3D", expiryDate.split("/")[0]);
            inputParams.put("CID3M", expiryDate.split("/")[1]);
            inputParams.put("CID3Y", expiryDate.split("/")[2]);
            inputParams.put("CDOB", dateOfBirth);
            inputParams.put("CDBD", dateOfBirth.split("/")[0]);
            inputParams.put("CDBM", dateOfBirth.split("/")[1]);
            inputParams.put("CDBY", dateOfBirth.split("/")[2]);
    ...
    private Map<String, String> createRequestForUpdateCustomerApplicationDataService(Result getCustomerApplicationData, Result getScoreCardS2, Result getScoreCardS3) {
        ...
        try {
            ...
            inputParams.put("softdeleteflag", HelperMethods.getFieldValue(getCustomerApplicationData, "softdeleteflag"));
            ...
            inputParams.put("loanAmountCap", getScoreCardS3.getParamValueByName("loanAmountCap"));
            inputParams.put("loanRate", getScoreCardS3.getParamValueByName("loanRate"));
        } catch (Exception ex) {
    ...
    private void calculateMonthlyNetSalary(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            switch (EMPLOYER_TYPE_ID) {
                ...
                int payMonth = Integer.parseInt(getSalaryCertificate.getParamValueByName("Paymonth"));
                if (payMonth < lastMonth) {
                    // Call Simah Salary Certificate with EmployeerId = 3
                } else {
                    MONTHLY_NET_SALARY = inputParams.get("netSalary");
                }
                */
                ...
                case "3":
                    ...
                    if (EMPLOYMENT_STATUS.equalsIgnoreCase("Active")) {
                        ...
                        if (NATIONALITY.equalsIgnoreCase("SAU")) {
                            double minimumAmount = 0.1 * (Integer.parseInt(getSalaryCertificate.getParamValueByName("basicSalary")) + Integer.parseInt(getSalaryCertificate.getParamValueByName("housingAllowance")));
                            calculatedDeductions = minimumAmount < 4500 ? minimumAmount : 4500;
                        }
                        MONTHLY_NET_SALARY = String.valueOf((Integer.parseInt(getSalaryCertificate.getParamValueByName("basicSalary")) + Integer.parseInt(getSalaryCertificate.getParamValueByName("housingAllowance")) + Integer.parseInt(getSalaryCertificate.getParamValueByName("otherAllowance"))) - calculatedDeductions);
                    }
                    break;
                default:
        ...
        try {
            switch (EMPLOYER_TYPE_ID) {
                case "1":
                    ...
                    CURRENT_LENGTH_OF_SERVICE = String.valueOf(Math.toIntExact(ChronoUnit.MONTHS.between(
                            YearMonth.from(agencyEmploymentDate),
                ...
                case "3":
                    ...
                    CURRENT_LENGTH_OF_SERVICE = String.valueOf(Math.toIntExact(ChronoUnit.MONTHS.between(
                            YearMonth.from(dateOfJoining),
                            YearMonth.from(currentDateNow)
                    )));
                    break;
                default:
    ...
    private void calculateGlobalDTI(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            String productType = productDetailRecord.getParamValueByName("CI_PRD");
            String status = productDetailRecord.getParamValueByName("CI_STATUS");

            if (!Arrays.asList(NON_FINANCIAL_PRODUCTS).contains(productType) && status.equalsIgnoreCase("A")) {
                // Financial product
                String installment = productDetailRecord.getParamValueByName("CI_INSTL");
                String frequency = productDetailRecord.getParamValueByName("CI_FRQ");
                String creditLimit = productDetailRecord.getParamValueByName("CI_LIMIT");
    ...
    private void calculateInternalDTI(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            Record productDetailRecord = getConsumerEnquiry.getRecordById("CI_DETAIL");
            String productType = productDetailRecord.getParamValueByName("CI_PRD");
            String productCode = productDetailRecord.getParamValueByName("CI_CRDTR");
            String status = productDetailRecord.getParamValueByName("CI_STATUS");
    ...
    private void calculateCurrentDelinquencyAndCurrentDelinquencyT(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            CURRENT_DELINQUENCY = "0";
            CURRENT_DELINQUENCY_T = "1";
            Record productDetailRecord = getConsumerEnquiry.getRecordById("CI_DETAIL");
            String productType = productDetailRecord.getParamValueByName("CI_PRD");
            String summary = productDetailRecord.getParamValueByName("CI_SUMMRY");
    ...
    private void calculateFinancialDefaultAmount(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            String productType = productDetailRecord.getParamValueByName("DF_PRD");

            if (!Arrays.asList(NON_FINANCIAL_PRODUCTS).contains(productType)) {
                FINANCIAL_DEFAULT_AMOUNT = productDetailRecord.getParamValueByName("DF_CUB");
            }
        } catch (Exception ex) {
    ...
    private void calculateNonFinancialDefaultAmount(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            if (Arrays.asList(NON_FINANCIAL_PRODUCTS).contains(productType)) {
                NON_FINANCIAL_DEFAULT_AMOUNT = productDetailRecord.getParamValueByName("DF_CUB");
            }
        } catch (Exception ex) {
    ...
    private void calculateBouncedCheque(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            String bouncedChequeValue = bouncedChequeRecord.getParamValueByName("BC_SETTLE_DATE");
            BOUNCED_CHEQUE = "NB";
        } catch (Exception ex) {
    ...
    private void calculateCourtJudgement(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            String courtJudgementValue = courtJudgementRecord.getParamValueByName("EJ_SETTLE_DATE");
            COURT_JUDGEMENT = "NJ";
        } catch (Exception ex) {
    ...
    private void getEmployerName(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            ...
            /*
            int currentLengthOfService = Integer.parseInt(CURRENT_LENGTH_OF_SERVICE);
            if (currentLengthOfService <= 3) {
                EMPLOYER_CATEGORISATION = "G";
            } else if (currentLengthOfService <= 6) {
                EMPLOYER_CATEGORISATION = "C";
            } else {
    ...
    private void calculateManagingSeasonalAndTemporaryLiftInSalary(Result getConsumerEnquiry, Result
            getSalaryCertificate) {
        try {
            int basicSalary = Integer.parseInt(getSalaryCertificate.getParamValueByName("basicSalary"));
            int OtherAllowance = 0;
            MANAGING_SEASONAL_AND_TEMPORARY_LIFT_IN_SALARY = "1";
            switch (EMPLOYER_TYPE_ID) {
                case "1": //Govt
                    OtherAllowance = Integer.parseInt(getSalaryCertificate.getParamValueByName("totalAllowance"));
                    break;
                case "3": //Private
    ...
    private void calculateSalaryWithoutAllowances(Result getConsumerEnquiry, Result getSalaryCertificate) {
        try {
            switch (EMPLOYER_TYPE_ID) {
                case "1": //Govt
                    SALARY_WITHOUT_ALLOWANCES = String.valueOf(Integer.parseInt(MONTHLY_NET_SALARY) - Integer.parseInt(getSalaryCertificate.getParamValueByName("totalAllowance")));
                    break;
                case "3": //Private
                    SALARY_WITHOUT_ALLOWANCES = String.valueOf(Integer.parseInt(MONTHLY_NET_SALARY) - Integer.parseInt(getSalaryCertificate.getParamValueByName("otherAllowance")));
                    break;
            }
        } catch (Exception ex) {
    ...
    private boolean preProcess(Map<String, String> inputParams) {
        try {
            APPLICATION_ID = inputParams.get("applicationID");
            NATIONAL_ID = inputParams.get("nationalID");
            DATA_TYPE = inputParams.get("Product");
            CONTACT_NUMBER = inputParams.get("Mobile");
    ...
    private void extractValuesFromCustomerApplication(Result getCustomerApplicationData) {
        try {
            ...
            //LOAN_REF = HelperMethods.getFieldValue(getCustomerApplicationData, "loanRef");
    ...
    private Result getCustomerData(DataControllerRequest dataControllerRequest) {
        ...
        try {
            ...
            getCustomerData.appendResult(ServiceCaller.internalDB(DBXDB_SERVICES_SERVICE_ID, CUSTOMER_GET_OPERATION_ID, filter, null, dataControllerRequest));
    ...
    private Result getSIMAHConsumerEnquiry(Map<String, String> inputParams, DataControllerRequest
            dataControllerRequest) {
        Result result = StatusEnum.error.setStatus();
        try {
            Result getConsumerEnquiry = StatusEnum.success.setStatus();
            getConsumerEnquiry.appendResult(ServiceCaller.internal(SIMAH_SERVICE_ID, CONSUMER_ENQUIRY_OPERATION_ID, inputParams, null, dataControllerRequest));
    ...
    private Result getSIMAHSalaryCertificate(Map<String, String> inputParams, DataControllerRequest
            dataControllerRequest) {
        Result result = StatusEnum.error.setStatus();
        try {
            Result getSalaryCertificate = StatusEnum.success.setStatus();
            getSalaryCertificate.appendResult(ServiceCaller.internal(SIMAH_SALARY_ORCH_SERVICE_ID, SIMAH_SALARY_CERT_OPERATION_ID, inputParams, null, dataControllerRequest));
            String inputRequest = (new ObjectMapper()).writeValueAsString(inputParams);
    ...
    private Result getNationalAddress(Map<String, String> inputParams, DataControllerRequest
        ...
        try {
            ...
            inputParams.put("userName", NATIONAL_ID);
            inputParams.put("dateOfBirth", DOB);
            getNationalAddress.appendResult(ServiceCaller.internal(YAKEEN_SOAP_API_SERVICE_ID, GET_CITIZEN_ADDRESS_INFO_OPERATION_ID, inputParams, null, dataControllerRequest));
            String inputRequest = (new ObjectMapper()).writeValueAsString(inputParams);
    ...
    private Result calculateScoreCardS2(Map<String, String> inputParams, DataControllerRequest
        ...
        try {
            ...
            getScoreCardS2.appendResult(ServiceCaller.internal(KNOCKOUT_SERVICE_ID, CALCULATE_SCORECARD_S2_OPERATION_ID, inputParams, null, dataControllerRequest));
            String inputRequest = (new ObjectMapper()).writeValueAsString(inputParams);
    ...
    private Result calculateScoreCardS3(Map<String, String> inputParams, DataControllerRequest
        ...
        try {
            ...
            getScoreCardS3.appendResult(ServiceCaller.internal(KNOCKOUT_SERVICE_ID, CALCULATE_SCORECARD_S3_OPERATION_ID, inputParams, null, dataControllerRequest));
            String inputRequest = (new ObjectMapper()).writeValueAsString(inputParams);
    ...
    private Result updateCustomerApplicationData(Map<String, String> inputParams, DataControllerRequest
            dataControllerRequest) {
        Result result = StatusEnum.error.setStatus();
        try {
            ...
            LOG.error("ERROR updateCustomerApplicationData :: " + ex);
        }
        return result;
    }
