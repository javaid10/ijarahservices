...
public class ScoringEngine implements JavaService2 {
	...
	public Object invoke(String s, Object[] objects, DataControllerRequest dataControllerRequest,
		...
		if (preProcess(dataControllerRequest)) {
			...
			// HS If no Pay Month is present IT means its a private employee
			else {
			...
			// Result getNationalAddress = getNationalAddress(inputParams,
			// dataControllerRequest);
			...
			calculateCurrentDelinquencyAndCurrentDelinquencyT(); // There are multiple products so of which product we
																	// need to calculate current delinquency
			calculateMaxDelinquency(); // There are multiple products so of which product we need to calculate max
										// delinquency
	...
	private Map<String, String> createRequestForUpdateCustomerApplicationDataServiceS2(
			Result getCustomerApplicationData, Result getScoreCardS2) {
	...
	private Map<String, String> createRequestForUpdateCustomerApplicationDataService(Result getCustomerApplicationData,
		...
		if (!loanRate.equalsIgnoreCase("0")) {
			...
			double amountOffer = Math.min(
					...
					Double.parseDouble(inputParams.get("loanAmount").replaceAll(",", "")));
	...
	private void calculateMonthlyNetSalary(Result getSalaryCertificate) {
		try {
			...
			case "3":
				...
				if (EMPLOYMENT_STATUS == "نشيط" || EMPLOYMENT_STATUS.equalsIgnoreCase("Active")) {
	...
	private void calculateMaxGlobalDTI() {
		LOG.error("calculateMaxGlobalDTI :: " + MONTHLY_NET_SALARY);
